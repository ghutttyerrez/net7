...
export default function CoverageModal({ open, onClose }) {
  ...
  return (
    <ModalPortal>
      <div className="fixed inset-0 z-[70] flex items-start justify-center p-4 md:p-8 overflow-y-auto" role="dialog" aria-modal="true" aria-labelledby="coverage-modal-title">
        <div className="absolute inset-0 bg-black/70 backdrop-blur-sm" onClick={()=>{ trackEvent('coverage_close_backdrop'); onClose() }} />
        <div ref={dialogRef} className="relative w-full max-w-lg glass rounded-2xl p-6 md:p-8 border border-black/10 dark:border-white/10 shadow-xl space-y-6 animate-fade-in focus:outline-none my-8">
